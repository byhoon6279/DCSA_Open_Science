...
DEFAULT_OUTPUT_DIR = REPO_ROOT / "RQ1" / "RQ1-3" / "results" / "family_aligned_baseline"
...
def summarize_group_metrics(run_results: List[Dict[str, object]]) -> Tuple[Dict[str, Dict[str, dict]], Dict[str, Dict[str, dict]]]:
...
def summarize_group_metrics_by_key(
    ...
    for key_value, rows in grouped.items():
        metrics, ranks = summarize_group_metrics(rows)
        out[key_value] = {"aggregate_metrics": metrics, "aggregate_ranks": ranks}
    return out


def weekly_rows(run_results: List[Dict[str, object]]) -> List[Dict[str, object]]:
...
def run_from_config(config: dict, config_path: Path | None = None) -> dict:
    ...
    aggregate_metrics, aggregate_ranks = summarize_group_metrics(run_results)
    aggregate_by_k = summarize_group_metrics_by_key(run_results, "k")
    return {
        "config": {
            **config,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--model", choices=["logistic_regression", "lightgbm"])
...
def main() -> None:
    ...
    weekly = weekly_rows(results["run_results"])
    if weekly:
    ...
    aggregate_by_k = aggregate_rows_by_grouping(results["aggregate_by_k"], "k")
    if aggregate_by_k:
