...
DEFAULT_OUTPUT_DIR = Path(__file__).resolve().parents[2] / "results" / "feature_subset_generalization"
...
def summarize_results(run_results: List[Dict[str, object]]) -> Tuple[Dict[str, Dict[str, dict]], Dict[str, Dict[str, dict]]]:
    ...
    for group in FEATURE_GROUPS:
        aggregate_metrics[group] = {
            ...
            for metric in ["acc", "f1", "auc", "positive_rate", "mix_at_k", "silhouette", "js_divergence"]
        }
...
def summarize_results_by_key(
    ...
    for key_value, rows in grouped.items():
        metrics, ranks = summarize_results(rows)
        out[key_value] = {"aggregate_metrics": metrics, "aggregate_ranks": ranks}
    return out


def feature_views_by_group(samples: Sequence) -> Dict[str, np.ndarray]:
    return {
...
def run_seed_experiment(
    ...
    train_features_by_group = feature_views_by_group(train_samples)
    ...
    for week_idx, week in enumerate(test_weeks):
        ...
        if balance_test:
            ...
            test_features_by_group = feature_views_by_group(test_samples)
        else:
...
def run_from_config(config: dict) -> dict:
    ...
    if not balance_test:
        for week, samples in test_samples_by_week.items():
            cached_unbalanced_test_views_by_week[week] = feature_views_by_group(samples)
    ...
    aggregate_metrics, aggregate_ranks = summarize_results(run_results)
    aggregate_by_seed = summarize_results_by_key(run_results, "seed")
    aggregate_by_mix_k = summarize_results_by_key(run_results, "mix_k")
...
def main() -> None:
    ...
    parser.add_argument("--model", choices=["logistic_regression", "lightgbm"], help="Override config model_type.")
