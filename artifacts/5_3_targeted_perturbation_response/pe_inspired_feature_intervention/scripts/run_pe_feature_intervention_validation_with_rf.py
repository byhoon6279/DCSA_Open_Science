#!/usr/bin/env python3
"""
RQ2-4: PE-Inspired Feature Intervention Validation (with Random Forest support)

Forked from `run_pe_feature_intervention_validation.py`. The only differences
from the base script are: a native `model_type="random_forest"` branch in
`fit_classifier`/`compute_feature_importance` (added below), and the
`rf_n_estimators`/`rf_n_jobs` config/CLI plumbing needed to drive it. The
existing `logistic_regression`/`lightgbm` code paths are unchanged.

`compute_feature_importance` supports both `importance_method="permutation"`
and `importance_method="gain"` for `model_type="random_forest"` (the latter
via `RandomForestClassifier.feature_importances_`, reusing the same generic
fallback the `lightgbm` branch already uses for non-booster classifiers).
`gain` is included here because it is the `importance_method` declared by
this section's committed `pe_intervention_*_random_forest.json` configs and
their already-committed results — this restores the declared method, it
does not by itself confirm that a rerun reproduces the committed numbers
bit-for-bit; that can only be checked with an actual EMBER2024 run.
"""
...
def compute_feature_importance(
    *,
    ...
    if model_type == "random_forest" and importance_method not in {"permutation", "gain"}:
        raise ValueError(
            "Random Forest in RQ2-4 supports importance_method in {'permutation', 'gain'} "
            "('gain' uses RandomForestClassifier.feature_importances_, the impurity-based "
            "importance declared by this section's committed configs)."
        )
...
def run_from_config(config: dict) -> dict:
    ...
    if num_workers < 1 or lgbm_n_jobs < 1 or omp_num_threads < 1 or rf_n_estimators < 1:
        raise ValueError("num_workers, lgbm_n_jobs, omp_num_threads, and rf_n_estimators must all be >= 1.")
    ...
    for seed in seeds:
        ...
        if model_type in {"lightgbm", "random_forest"} and importance_method == "permutation":
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--rf-n-estimators", type=int, help="Override config rf_n_estimators.")
    parser.add_argument("--rf-n-jobs", type=int, help="Override config rf_n_jobs.")
...
def main() -> None:
    ...
    if args.rf_n_estimators is not None:
        config["rf_n_estimators"] = args.rf_n_estimators
    if args.rf_n_jobs is not None:
        config["rf_n_jobs"] = args.rf_n_jobs
    data_root = Path(config["data_root"])
