...
# produce the bundled D3 class-conditional sensitivity results in ../results/. The
...
def load_records_from_jsonl(path: Path, packer_filter: str) -> List[SampleRecord]:
    ...
    with path.open("r", encoding="utf-8") as handle:
        for line in handle:
            ...
            if isinstance(packer, list):
                packed = len(packer) > 0
            elif isinstance(packer, str):
                packed = bool(packer.strip())
...
def load_records_from_paths(paths: Sequence[Path], packer_filter: str) -> List[SampleRecord]:
    rows: List[SampleRecord] = []
    for path in paths:
        rows.extend(load_records_from_jsonl(path, packer_filter))
    return rows
...
def balance_records(
    ...
    benign = [record for record in records if record.sample.label == 0]
    malware = [record for record in records if record.sample.label == 1]
    target = min(len(benign), len(malware))
    if max_per_class is not None:
        target = min(target, max_per_class)
    rng = np.random.default_rng(seed)

    def pick(rows: List[SampleRecord]) -> List[SampleRecord]:
        if len(rows) <= target:
            return list(rows)
...
def class_conditional_metric_row(
    *,
    seed: int,
    feature_group: str,
    density_bin: str,
    ...
    X_scaled: np.ndarray,
    y: np.ndarray,
    sample_mix: np.ndarray,
    clf: Any,
) -> Dict[str, object]:
    benign_mask = y == 0
    malware_mask = y == 1
    benign_mean = float(sample_mix[benign_mask].mean()) if benign_mask.any() else float("nan")
    malware_mean = float(sample_mix[malware_mask].mean()) if malware_mask.any() else float("nan")
    balanced_values = [value for value in [benign_mean, malware_mean] if not math.isnan(value)]
    class_balanced_mean = (
        float(sum(balanced_values) / len(balanced_values)) if balanced_values else float("nan")
    )
    return {
        "seed": seed,
        "density_bin": density_bin,
        ...
        "auc": compute_auc(clf, X_scaled, y),
        "mix_at_10": float(sample_mix.mean()) if len(sample_mix) else float("nan"),
        "mix_at_10_benign_query": benign_mean,
        "mix_at_10_malware_query": malware_mean,
        "mix_at_10_class_balanced": class_balanced_mean,
        "js_divergence": js_divergence_feature_mass(X_raw, y),
    }
...
def process_seed_block1(
    *,
    ...
    all_test_records_by_week: Dict[str, List[SampleRecord]],
    ...
    results_by_k: Dict[int, Dict[str, List[Dict[str, object]]]] = {
        k_density: {
            "density_rows": [],
            "composition_rows": [],
            "metric_rows": [],
    ...
    if balance_train:
        train_records = balance_records(all_train_records, seed=seed, max_per_class=max_train_per_class)
    pooled_test_records: List[SampleRecord] = []
    for week_idx, week in enumerate(test_weeks):
        records = all_test_records_by_week[week]
        if balance_test:
        ...
        pooled_test_records.extend(records)
    ...
    density_cache: Dict[int, tuple[np.ndarray, np.ndarray, Dict[str, float]]] = {}
    for k_density in k_values:
        density_scores = compute_density_scores(X_train_all_scaled, X_test_all_scaled, k_density=k_density)
        density_bins, thresholds = assign_density_bins(density_scores)
        density_cache[k_density] = (density_scores, density_bins, thresholds)
    ...
    for k_density in k_values:
        density_scores, density_bins, thresholds = density_cache[k_density]
        density_rows = results_by_k[k_density]["density_rows"]
        composition_rows = results_by_k[k_density]["composition_rows"]
        metric_rows = results_by_k[k_density]["metric_rows"]
        ranking_rows = results_by_k[k_density]["ranking_rows"]
        class_conditional_composition_rows = results_by_k[k_density]["class_conditional_composition_rows"]
        class_conditional_metric_rows = results_by_k[k_density]["class_conditional_metric_rows"]
        ...
        if save_class_conditional_rows:
            ...
            for density_bin in ["high_density", "mid_density", "low_density"]:
                ...
                row = composition_row(seed, density_bin, selected)
        ...
        for group in feature_groups:
            ...
            if save_class_conditional_rows and class_conditional_bins is not None:
                for density_bin in ["high_density", "mid_density", "low_density"]:
                    mask = class_conditional_bins == density_bin
                    row = class_conditional_metric_row(
                        seed=seed,
                        feature_group=group,
                        density_bin=density_bin,
                        X_raw=X_test_raw[mask],
                        X_scaled=X_test_scaled[mask],
                        y=y_test[mask],
                        sample_mix=sample_mix[mask],
                        clf=clf,
                    )
                    row["density_definition"] = "class_conditional"
                    row["thresholds_json"] = json.dumps(class_conditional_thresholds, sort_keys=True)
                    class_conditional_metric_rows.append(row)
...
def run_from_config(config: dict) -> dict:
    ...
    all_train_records = load_records_from_paths(train_paths, packer_filter=packer_filter)
    all_test_records_by_week = {
        week: load_records_from_jsonl(
    ...
    if num_workers == 1:
        seed_results_iter: Iterable[Dict[int, Dict[str, List[Dict[str, object]]]]] = (
            process_seed_block1(
                seed=seed,
        ...
        for seed_results in seed_results_iter:
            for k_density in k_values:
                for key in [
                    ...
                    results_by_k[k_density][key].extend(seed_results[k_density][key])
    else:
    ...
    final_results_by_k: Dict[int, dict] = {}
    for k_density in k_values:
        density_rows = results_by_k[k_density]["density_rows"]
        composition_rows = results_by_k[k_density]["composition_rows"]
        metric_rows = results_by_k[k_density]["metric_rows"]
        ...
        class_conditional_composition_rows = results_by_k[k_density]["class_conditional_composition_rows"]
        class_conditional_metric_rows = results_by_k[k_density]["class_conditional_metric_rows"]
        final_results_by_k[k_density] = {
            "config": {**config, "k_density": k_density},
            "metric_notes": {
            ...
            "class_conditional_metric_rows": class_conditional_metric_rows,
            ...
            "aggregate_class_conditional_composition_rows": aggregate_rows(
                class_conditional_composition_rows,
                group_keys=["density_bin", "density_definition"],
                metric_keys=["n_samples", "n_benign", "n_malware", "benign_to_malware_ratio", "n_packed_malware", "packed_ratio"],
            ) if class_conditional_composition_rows else [],
            "aggregate_class_conditional_metric_rows": aggregate_rows(
                class_conditional_metric_rows,
                group_keys=["density_bin", "feature_group", "density_definition"],
                metric_keys=[
                    "auc",
                    "mix_at_10",
                    "mix_at_10_benign_query",
                    "mix_at_10_malware_query",
                    "mix_at_10_class_balanced",
                    "js_divergence",
                    "n_samples",
                    "n_benign",
                    "n_malware",
                ],
            ) if class_conditional_metric_rows else [],
        }

    if len(k_values) == 1:
        return final_results_by_k[k_values[0]]
    return {"results_by_k": final_results_by_k, "config": config}
...
def save_results(results: dict, output_dir: Path) -> None:
    ...
    if results["class_conditional_composition_rows"]:
        write_csv(
            output_dir / "class_conditional_composition_rows.csv",
            list(results["class_conditional_composition_rows"][0].keys()),
            results["class_conditional_composition_rows"],
        )
    if results["class_conditional_metric_rows"]:
        write_csv(
            output_dir / "class_conditional_metric_rows.csv",
            list(results["class_conditional_metric_rows"][0].keys()),
            results["class_conditional_metric_rows"],
        )
    ...
    if results["aggregate_class_conditional_composition_rows"]:
        write_csv(
            output_dir / "aggregate_class_conditional_composition_rows.csv",
            list(results["aggregate_class_conditional_composition_rows"][0].keys()),
            results["aggregate_class_conditional_composition_rows"],
        )
    if results["aggregate_class_conditional_metric_rows"]:
        write_csv(
            output_dir / "aggregate_class_conditional_metric_rows.csv",
            list(results["aggregate_class_conditional_metric_rows"][0].keys()),
            results["aggregate_class_conditional_metric_rows"],
        )
    summary = {
        ...
        "n_class_conditional_metric_rows": len(results["class_conditional_metric_rows"]),
    }
...
def main() -> None:
    ...
    if "results_by_k" in results:
        for k_density, k_results in results["results_by_k"].items():
            save_results(k_results, args.output_dir / f"k_{k_density}")
    else:
